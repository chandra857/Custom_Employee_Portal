...
export async function POST(request: Request) {
  try {
    ...
    const recent = await database.prepare(`
      ...
      WHERE actor_email = ? AND action = 'auth.login' AND status = 'failed'
        AND created_at > EXTRACT(EPOCH FROM NOW())::integer - 300
    ...
    const account = await database.prepare('SELECT id, email, password_hash, password_salt, is_active FROM users WHERE email = ?').bind(email).first<{ id: number; email: string; password_hash: string; password_salt: string; is_active: number }>();
    ...
    await database.prepare('UPDATE users SET last_login_at = EXTRACT(EPOCH FROM NOW())::integer, updated_at = EXTRACT(EPOCH FROM NOW())::integer WHERE id = ?').bind(account.id).run();
